...
namespace sh {
	...
	public:
		...
		private:
			Task($<Task>& ptr, Options options, Function<Promise<void>($<Task>)> executor);
			
			Promise<void> perform();
			
			w$<Task> self;
			Options options;
			Function<Promise<void>($<Task>)> executor;
		...
		$<Task> getTaskWithName(const String& name);
		$<Task> getTaskWithTag(const String& tag);
		LinkedList<$<Task>> getTasksWithTag(const String& tag);
	...
	private:
		...
		Promise<void> performWork($<Task> task, Work work);
...
#pragma mark - AsyncQueue implementation
	...
	AsyncQueue::RunResult AsyncQueue::run(RunOptions options, Work work) {
		...
		auto task = new$<Task>(taskOptions, [=](auto task) {
		...
		taskQueue.pushBack(TaskNode{ .task=task, .promise=taskPromise });
	...
	Promise<void> AsyncQueue::performWork($<Task> task, Work work) {
